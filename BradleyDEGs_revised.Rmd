...
```
```{r}
TC.counts.normal.log.named <- TC.counts.normal.log
colnames(TC.counts.normal.log.named) <- c("C_019", "T_026", "T_027", "C_030", "C_045", "T_052","C_062", "T_087")

TC.counts.normal.log.named 

#I need to come back to fix this...might need to redo entire TC analysis as well by removing samples that did not have TC in it
```
```{r}
...
```{r, eval = FALSE}
#need to fix above code before this will work
...
```
```{r}
top10.PM.interaction <- rownames(topTags(interaction.lrt.PM, n = 15)$table)

...
ggplot(deg_results, aes(x = logFC, y = neg_log10_pval, color = significance)) +
  ...
  labs(
    x = "Log2 Fold Change",
    y = "-Log10 FDR",
    title = "DGE of Host Plant in Response to Drought x Inoculation Treatment"
  ) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") +  # FDR threshold
  geom_vline(xintercept = c(-0.5, 0.5), linetype = "dashed")  # Fold-change cutoff
...
ggplot(deg_results, aes(x = logFC, y = neg_log10_pval, color = significance)) +
  geom_point(alpha = 0.6) +
  geom_text_repel(
    data = deg_results %>% filter(significance == "Significant (FDR < 0.05)"),  
    aes(label = Gene), 
    max.overlaps = 15,
    size = 3
  ) +
...
new_gene_names <- c("CYP71", "auxin signaling", "sugar metabolism; stress/defense", "CRK", "TIR-NLR-like", "unkown", "sugar metabolism; stress", "specialized metabolism", "unknown", "bHLH", "DCN1", "RdRP", "glycogen + 2ary metabolism; stress", "USP", "MKS1-like", "cell signaling; metabolism; stress", "antibiotic synthesis", "GH16 XET cell wall mod", "pectin cell wall mod", "stress/defense", "cell signaling; post-translational mod", "aa + 2ary metabolism", "sugar transporter", "protein-protein", "cell signaling", "sugar transporter", "BURP", "USP", "XTH")

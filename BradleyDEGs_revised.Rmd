...
```{r}
summary(decideTestsDGE(trt.lrt.TC,p.value=0.05)) #This uses the FDR.  0.05 would be OK also.
summary(decideTestsDGE(trt.lrt.PM,p.value=0.05)) 
summary(decideTestsDGE(sp.lrt.PM,p.value=0.05))
...
gene_categories <- data.frame(
  Category = c("metabolism", "development", "metabolism", "stress response", "defense", "unknown", "metabolism", "metabolism", "unknown", "gene regulation", "development", "gene regulation", "metabolism", "stress response", "development", "metabolism", "metabolism", "cell wall mod", "cell wall mod", "defense", "metabolism", "metabolism", "transport", "development", "stress response", "transport", "cell wall mod", "stress response", "cell wall mod")
)
...
```{r}
png("finalheatmap.png", width = 1200, height = 1600, res = 300)
pheatmap(logfc_matrix,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         scale = "none",
...
```{r}
# Open a PDF file
pdf("colloquium.pdf", width = 8, height = 10)  # adjust width/height as needed
...
10/1/25: talked with Laura about DFD...we want to explore more analysis

analysis to do list:
- revisit FDR value decisions (some were 0.01, some 0.05, why?)
- Annotate DEGs of individual treatments (similar to what I did with
  interaction)
- attempt coexpression
- explore different ordination

Revisiting FDR value decisions and annotating sp and trt DEGs:

```{r}
#(top 29 genes for both trt and sp will be sig FDR = 0.05)
#filter DEGs for only the top 29 in each individual treatment (sp and trt)

top29.PM.sp <- rownames(topTags(sp.lrt.PM, n=29)$table)
top29.PM.sp.named <- PM.counts.normal.log.named[top29.PM.sp, ]

top29.PM.trt <- rownames(topTags(trt.lrt.PM, n = 29)$table)
top29.PM.trt.named <- PM.counts.normal.log.named[top29.PM.trt, ]

```

```{r}
#Extract genes with a FDR < 0.05
PM.29.trt.DEGs <- topTags(interaction.lrt.PM,n = 29, p.value = 0.05)$table
PM.29.sp.DEGs <- topTags(sp.lrt.PM, n = 29, p.value = 0.05)$table

#save to a file
write.csv(PM.29.trt.DEGs,"PM.29.trt.DEGs.csv")
write.csv(PM.29.sp.DEGs, "PM.29.sp.DEGs.csv")
```

Now that I have a dataframe with the top 29 treatment and species DEGs, the next step is to annotate them. I will use ChatGPT to explore ways I can do this with code.

Previously I manually entered 29 interaction genes into interpro website. That workflow is in "PM.interaction.DEGs.rmd"

I will make a new doc to annotate the 29 trt and sp genes called "trt.sp.annotation.rmd"

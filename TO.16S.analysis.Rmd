...
If the above code ever finishes, I can save my progress by outputting it as a csv that I can load if need be:

```{r}
write.csv(out, file = "filterAndTrim_output.csv", row.names = TRUE)
```
...
```{r}
errF <- read.csv("errF.csv")
errR <- read.csv("errR.csv")
```
...
```{r}
ps_relabund <- transform_sample_counts(ps_final, function(OTU) OTU/sum(OTU))

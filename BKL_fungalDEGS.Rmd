...
```{r}
fungal.counts <- fungal.counts[, -which(names(fungal.counts) == "target_id")]
...
```{r}
transformed.counts <- log2(fungal.counts + 1) 
#log transform counts
```

```{r}
cor.table <- cor(fungal.counts)
...
fungal.counts.new <- sample.description %>%
  mutate(
    sp = str_extract(sample, "(TC|TCS)"),  
    trt = str_extract(sample, "(C|T)"),     

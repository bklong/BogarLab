...
head A03_counts.tsv #replace this file with fungal counts that you get back from core
```

```{r}
library(readr)
fungal.counts <- read_tsv("../input/gh_internode_counts2.tsv") #assign fungal counts data to an object
```
...
fungal.counts <- fungal.counts %>% filter(gene_id!="*")
fungal.counts[is.na(fungal.counts)] <- 0
...
colnames(fungal.counts) <- str_remove(colnames(fungal.counts), fixed(".1_matched.merged.fq.bam"))

colnames(fungal.counts)
```
...
counts_subset <- fungal.counts[, -which(names(fungal.counts) == "gene_id")]
...
sample.description <- tibble(sample=colnames(fungal.counts)[-1])
...
fungal.counts.new <- sample.description %>%
  mutate(
...
head(fungal.counts.new)
```

```{r}
#change treatments according to our data
fungal.counts.new <- fungal.counts.new %>%
  mutate(gt=factor(gt), 
...
```{r}
library(edgeR)
counts.matrix <- fungal.counts %>% select(-gene_id) %>% as.matrix()
rownames(counts.matrix) <- fungal.counts$gene_id

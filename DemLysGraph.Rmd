...
last_and_third_to_last <- reshaped_data %>%
  group_by(ID) %>%
  filter(row_number() == n() | row_number() == n() - 2) %>%
  ungroup()
...
last_and_second_to_last <- reshaped_data %>%
  group_by(ID) %>%
  slice_tail(n = 2) %>%
  arrange(desc(row_number())) %>%  # Arrange in descending order to get second-to-last first
  ungroup()
...
hourly_loss <- last_and_third_to_last %>%
  ...
  summarize(
    ...
    massdiff_g = dplyr::first(sample) - dplyr::last(sample),
    first_time = dplyr::first(newtime),
    last_time = dplyr::last(newtime),
    timediff = dplyr::first(newtime) - dplyr::last(newtime)
  ) %>%
  mutate(
    mass_per_day = massdiff_g / as.numeric(timediff),
    mass_per_hr = mass_per_day / 24
  )
